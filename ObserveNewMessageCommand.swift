...
class ObserveNewMessagesCommand: ObserveCommand {
    var observer: NewMessagesObserver?
    ...
    func addObserver() {
        ...
        messageHandle = messageReference!.queryOrdered(byChild: "user").queryEqual(toValue: userId).observe(.childAdded) { (snapshot) in
...
//            }
            if (newMessage.value(forKey: "interpreter") as? String == self.interpreterId) {
                if (newMessage.value(forKey: "image") != nil) {
                    ...
                    self.observer?.handleNewMessage(message: message)
    ...
    func deleteObserver() {
        self.messageReference?.removeObserver(withHandle: messageHandle!)
    }
}
